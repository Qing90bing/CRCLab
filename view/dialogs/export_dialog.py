...
class ExportDialog:
    ...
    def _setup_bindings(self):
        ...
        def on_display_dir_changed(*_):
            ...
            dir_mode = form.dir_mode_var.get()
            if dir_mode == Config.EXPORT_OPTIONS["dir_modes"][1] and form.custom_dir_var.get() != form.display_dir_var.get():
    ...
    def _update_export_info(self, img, data, dividend, divisor, q, rows, ctx):
        ...
        multiplier = {opt_q[0]: 1, opt_q[1]: 2, opt_q[2]: 3, opt_q[3]: 4}.get(form.quality_var.get(), 1)
                ...
                try:
                    if fmt in ("svg", "pdf", "emf"):
                        size_bytes, w, h = Exporter.estimate_vector_size(
                            self.app, fmt, data, dividend, divisor, q, rows, ctx, form.color_var.get(), form.border_var.get()
                        )
                    else:
                        ...
                        size_bytes, w, h = Exporter.calculate_precise_bitmap_size(
                            ...
                            save_fmt,
                            form.dpi_var.get(),
                            round(float(form.jpg_quality_var.get())),
                        )
                    ...
                    def fail_ui():
                        if getattr(self, "_is_exporting", False):
                            ...
                            return
                        if getattr(self, "_current_calc_id", None) == calc_id:
                            form.size_lbl.config(text="预估大小： 估算失败")
    ...
    def _on_export_success(self, out_path, export_dir, width, height):
        ...
        transparent_bg = "是" if fmt.lower() in ("png", "svg") and is_transparent else "否"
        ...
        if is_vector:
            ...
            if fmt.lower() in ("jpg", "jpeg"):
                details["JPG 质量"] = str(round(float(form.jpg_quality_var.get()))) + "%"
    ...
    def export_chart(self):
        ...
        color_mode = form.color_var.get()
        quality = form.quality_var.get()
        jpg_quality = round(float(form.jpg_quality_var.get()))
        ...
        def async_worker():
            try:
                out_path, export_dir, width, height = Exporter.export(
                    snap, filename, fmt, show_border, color_mode, quality, jpg_quality, dpi, dir_mode, custom_dir
                )
                # 成功回调
    ...
    def _show_error_dialog(self, e):
        """格式化错误反馈"""
        messagebox.showerror(
            Config.MESSAGES["export_fail_title"], Config.MESSAGES["export_fail_body"].format(error_type=type(e).__name__, error_msg=str(e))
        )
