...
from view.components.widgets import LabeledCombobox, LabeledGroup, ReadOnlyPathEntry
...
class ExportForm(tk.Frame):
    ...
    def __init__(self, parent, dialog):
        ...
        # 1. 区域主标题描述
        tk.Label(self.top_content_container, text=Config.UI_TEXT["export_params"], bg=self.bg_color, font=Config.FONTS["zh_bold"]).pack(
    ...
    def _build_form_widgets(self):
        ...
        self.fmt_combo = LabeledCombobox(
            col1_1, Config.UI_TEXT["export_format"], self.fmt_var, Config.EXPORT_OPTIONS["formats"], bg=self.bg_color
        )
        self.fmt_combo.pack(fill=tk.X, expand=True)
        self.quality_combo = LabeledCombobox(
            col1_2, Config.UI_TEXT["export_quality"], self.quality_var, Config.EXPORT_OPTIONS["qualities"], bg=self.bg_color
        )
        ...
        self.dpi_combo = LabeledCombobox(
            col2_1, Config.UI_TEXT["export_dpi"], self.dpi_var, Config.EXPORT_OPTIONS["dpis"], bg=self.bg_color
        )
        self.dpi_combo.pack(fill=tk.X, expand=True)
        self.color_combo = LabeledCombobox(
            col2_2, Config.UI_TEXT["export_color"], self.color_var, Config.EXPORT_OPTIONS["colors"], bg=self.bg_color
        )
        self.color_combo.pack(fill=tk.X, expand=True)

        self.jpg_quality_label = tk.Label(
            spec_inner, text=self._format_jpg_quality_text(), bg=self.bg_color, font=Config.FONTS["zh_normal"], anchor="w"
        )
        self.jpg_quality_label.pack(fill=tk.X, pady=(2, 3))

        style = ttk.Style()
        style.configure("Export.Horizontal.TScale", background=self.bg_color, troughcolor=self.bg_color)
        ...
        tk.Label(filename_frame, text=Config.UI_TEXT["export_filename"], bg=self.bg_color, font=Config.FONTS["zh_normal"]).pack(
        ...
        # 导出路径及自定义选择区
        self.dir_mode_combo = LabeledCombobox(
            output_inner, Config.UI_TEXT["export_dir"], self.dir_mode_var, Config.EXPORT_OPTIONS["dir_modes"], bg=self.bg_color
        )
        self.dir_mode_combo.pack(fill=tk.X, expand=True)

        self.browse_btn = ttk.Button(
            output_inner, text=Config.UI_TEXT["export_btn_browse"], state=tk.DISABLED, command=self._pick_export_dir
        )
    ...
    def _build_info_and_actions(self):
        ...
        self.width_lbl = tk.Label(
            info_inner, text=Config.UI_TEXT["export_width_placeholder"], bg=self.bg_color, font=Config.FONTS["zh_normal"], anchor="w"
        )
        self.width_lbl.pack(fill=tk.X, pady=2)

        self.height_lbl = tk.Label(
            info_inner, text=Config.UI_TEXT["export_height_placeholder"], bg=self.bg_color, font=Config.FONTS["zh_normal"], anchor="w"
        )
        self.height_lbl.pack(fill=tk.X, pady=2)

        self.size_lbl = tk.Label(
            info_inner, text=Config.UI_TEXT["export_size_placeholder"], bg=self.bg_color, font=Config.FONTS["zh_normal"], anchor="w"
        )
        ...
        ttk.Button(btn_frame, text=Config.UI_TEXT["btn_cancel"], command=self.dialog.dlg.destroy).pack(
            side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 6)
        )

        self.export_btn = ttk.Button(
            btn_frame, text=Config.UI_TEXT["btn_start_export"], command=self.dialog.export_chart, style="Action.TButton"
        )
        self.export_btn.pack(side=tk.RIGHT, fill=tk.X, expand=True, padx=(6, 0))

    def _format_jpg_quality_text(self):
        """生成 JPG 质量滑块的展示文本。"""
        value = round(float(self.jpg_quality_var.get()))
        if value < 50:
            level = "低"
        elif value < 80:
            level = "中"
    ...
    def _on_jpg_quality_pointer(self, event):
        ...
        value = round(low + max(0.0, min(1.0, ratio)) * (high - low))
        self.jpg_quality_var.set(value)
    ...
    def show_filename_error(self, msg):
        ...
        lbl = tk.Label(tip, text="⚠️ " + msg, bg="#fef2f2", fg="#b91c1c", font=Config.FONTS["zh_normal"], justify=tk.LEFT, padx=8, pady=4)
        lbl.pack()
