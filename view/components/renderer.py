...
class CanvasRenderer:
    ...
    def _load_font(self, size, is_bold=False):
        ...
        for family in target_families:
            ...
            except OSError:  # noqa: PERF203  # intentional: try each font family in order
                continue
    ...
    def _render_raw_formula(self, data, dividend, divisor, q, rows, ctx_ssaa, ssaa_factor):
        ...
        if w_real > 0 and h_real > 0 and ssaa_factor > 1:
    ...
    def _draw_header_elements(self, draw, dividend, L, ctx, ox, oy):
        ...
        if not hasattr(draw, "commands") and hasattr(
            actual_draw, "ellipse"
        ):  # 仅针对原生 PIL 画布执行物理圆盖修补，不污染矢量拦截器（SVG、EMF 已由矢量引擎原生完美平滑对接）
            r = L["line_w"] / 2.0
            for pt in pts[1:-1]:
                x, y = pt
    ...
    def _draw_steps(self, draw, rows, data, line_y, L, ctx, ox, oy):
        ...
        for row in rows:
            if row["type"] == "line":
                ...
                lx0 = (
                    ox
                    + (L["pad_cells"] + row["offset"]) * L["cell_w"]
                    + L["grid_base"] * Config.LAYOUT["block_inset_ratio"]
                    + ctx["ext_left"] * L["grid_base"]
                )
                lx1 = (
                    ox
                    + (L["pad_cells"] + row["offset"] + row["len"]) * L["cell_w"]
                    - L["grid_base"] * Config.LAYOUT["block_inset_ratio"]
                    + ctx["ext_right"] * L["grid_base"]
                )
                ly = oy + curr_y
        ...
        if ctx["bg_block_color"] not in ("transparent", "none") and row["type"] == "remainder":
            if not is_verify:
                ...
                if 0 <= pad_s < len(row["val"]):
                    ...
                    bx1 = (
                        ox
                        + (L["pad_cells"] + row["offset"] + len(row["val"])) * L["cell_w"]
                        - L["grid_base"] * Config.LAYOUT["block_inset_ratio"]
                    )
                ...
                for bi, bchar in enumerate(row["val"]):
                    if bchar == "1":
                        ...
                        bx1 = (
                            ox
                            + (L["pad_cells"] + row["offset"] + bi + 1) * L["cell_w"]
                            - L["grid_base"] * Config.LAYOUT["block_inset_ratio"]
                        )
                        by0 = oy + cy_base + L["cell_h"] * 0.05
                        by1 = oy + cy_base + L["cell_h"] * 0.95
