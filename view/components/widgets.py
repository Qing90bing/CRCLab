...
class ReadonlyEntry(tk.Entry):
    ...
    def __init__(self, parent, text_val, font, fg, bg, width=24, justify: Justify = "right"):
        ...
        super().__init__(parent, relief="flat", state="normal", font=font, fg=fg, bg=bg, width=width, highlightthickness=0, justify=justify)
...
class ModernScale(tk.Frame):
    ...
    def __init__(self, parent, label, from_, to, var, resolution=1, command=None, bg=None):
        ...
        self.val_lbl = tk.Label(
            header, text=self._format_value(var.get()), bg=bg_color, fg=Config.COLORS["primary"], font=Config.FONTS["en_main"]
        )
        ...
        # 减号按钮
        self.dec_btn = ttk.Button(control_frame, text="－", style="ScaleBtn.TButton", command=self._decrement)
        self.dec_btn.pack(side=tk.LEFT, padx=(0, 5))

        # 使用 ttk.Scale 以获得更好的滑块样式
        self.scale = ttk.Scale(control_frame, from_=from_, to=to, orient=tk.HORIZONTAL, variable=var, command=self._on_scale_move)
        self.scale.pack(side=tk.LEFT, expand=True, fill=tk.X, padx=5)

        # 加号按钮
        self.inc_btn = ttk.Button(control_frame, text="＋", style="ScaleBtn.TButton", command=self._increment)
...
class ColorSwatchRow(tk.Frame):
    ...
    def __init__(
        ...
        if self.allow_transparent:
            ...
            self.trans_check = ttk.Checkbutton(self, text="透明", variable=self.is_transparent_var, command=self._on_trans_toggle)
        ...
        if bold_var:
            ...
            self.bold_check = ttk.Checkbutton(self, text="加粗", variable=bold_var, command=bold_command)
        ...
        for x in range(0, w, size):
            for y in range(0, h, size):
                if ((x // size) + (y // size)) % 2 == 1:
                    self.canvas.create_rectangle(x + 1, y + 1, x + size + 1, y + size + 1, fill="#e2e8f0", outline="", tags="checker")
...
class LabeledCombobox(tk.Frame):
    ...
    def __init__(self, parent, label_text, var, values, bg=None):
        ...
        self.label_widget = tk.Label(self, text=label_text, bg=bg_color, font=Config.FONTS["zh_normal"])
...
class ReadOnlyPathEntry(tk.Frame):
    ...
    def __init__(self, parent, textvariable):
        super().__init__(parent, bg="#f8fafc", highlightthickness=1, highlightbackground="#cbd5e1", padx=10, pady=8)
        self.entry = tk.Entry(
            self,
    ...
    def set_mode(self, is_custom):
        ...
        if not is_custom:
            ...
            self.entry.config(state="disabled", disabledbackground="#f1f5f9", disabledforeground="#94a3b8")
        else:
            self.config(bg="#ffffff", highlightbackground="#cbd5e1")
            self.entry.config(state="normal", background="#ffffff", foreground="#1e293b", selectbackground="#cbd5e1")
