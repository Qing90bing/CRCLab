...
from view.components.widgets import ColorSwatchRow, ModernScale
...
class SidebarPanel(tk.Frame):
    ...
    def _build_scrollable_container(self, side_w):
        ...
        tk.Label(
            title_row,
            text=Config.UI_TEXT["sidebar_title"],
            bg=Config.COLORS["sidebar_bg"],
            fg=Config.COLORS["sidebar_title_fg"],
            font=Config.FONTS["side_title"],
        ).grid(row=0, column=1)
        ...
        # 实例化内部配置容器
        self.inner_panel = tk.Frame(
            self.scrollable_frame, bg=Config.COLORS["sidebar_bg"], padx=Config.LAYOUT["input_padx"], pady=Config.LAYOUT["input_pady"]
        )
    ...
    def _init_input_section(self):
        ...
        # 2. 生成多项式输入
        tk.Label(parent, text=Config.UI_TEXT["poly_label"], bg=Config.COLORS["sidebar_bg"], font=Config.FONTS["zh_normal"]).pack(
        ...
        self.poly_combo.set(next(iter(Config.STD_POLYS.keys())))
    ...
    def _init_color_section(self):
        ...
        # 导出图表按钮（放在主 panel 中）
        ttk.Button(self.inner_panel, text=Config.UI_TEXT["btn_export"], command=self.app.open_export_dialog, style="Action.TButton").pack(
            fill=tk.X, pady=(15, 20)
        )
