...
class EMFInterceptDraw:
    ...
    def text(self, xy, text, fill="#000000", font=None, *args, **kwargs):
        ...
        gdi_font = self.gdi.CreateFontW(
            ...
            0,
            0,
            Config.GDI["default_charset"],
            0,
            0,
            Config.GDI["antialiased_quality"],
            0,
            Config.FONTS["gdi_family"],
        )
...
class EMFExporter(BaseExporter):
    ...
    def draw_to_emf(snap, hdc, data, dividend, divisor, q, rows, ctx):
        ...
        EMFExporter._draw_emf_elements(
            renderer, draw_proxy, data, q, dividend, divisor, rows, ctx_ssaa, L, ox, oy, x0, y0, x1, y1, p, ssaa_factor
        )
    ...
    def _draw_emf_elements(renderer, draw_proxy, data, q, dividend, divisor, rows, ctx_ssaa, L, ox, oy, x0, y0, x1, y1, p, ssaa_factor):
        ...
        if sheet_bg_color not in ("transparent", "none"):
            draw_proxy.rectangle(
                [x0 - p * ssaa_factor, y0 - p * ssaa_factor, x1 + p * ssaa_factor, y1 + p * ssaa_factor], fill=sheet_bg_color, outline=None
            )
