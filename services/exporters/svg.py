...
class SVGInterceptDraw:
    ...
    def text(self, xy, text, font=None, fill=None, anchor=None, *args, **kwargs):
        self.commands.append({"type": "text", "xy": xy, "text": text, "font": font, "fill": fill, "anchor": anchor})
        return self.real_draw.text(xy, text, *args, font=font, fill=fill, anchor=anchor, **kwargs)

    def line(self, xy, fill=None, width=1, joint=None, *args, **kwargs):
        self.commands.append({"type": "line", "xy": xy, "fill": fill, "width": width, "joint": joint})
        return self.real_draw.line(xy, *args, fill=fill, width=width, joint=joint, **kwargs)
    ...
    def rectangle(self, xy, fill=None, outline=None, width=1, *args, **kwargs):
        ...
        return self.real_draw.rectangle(xy, *args, fill=fill, outline=outline, width=width, **kwargs)
...
class SVGExporter(BaseExporter):
    ...
    def render_to_svg(renderer, data, dividend, divisor, q, rows, ctx):
        ...
        if ctx.get("show_border", True):
            ...
            svg_elements.append(
                f'  <rect x="0" y="0" width="{w_sheet}" height="{h_sheet}" fill="none" {border_attrs} stroke-width="{border_w:.2f}" />'
            )
